...
async function render(){
 ...
 if(state.user.admin)state.organizations=(await api('/organizations')).data;
...
function newTable(){modal('Criar tabela',field('name','Nome da tabela')+'<label class="check"><input name="organization_isolated" type="checkbox" checked>Isolar dados por organização</label><p>Depois de criar, adicione os campos e configure as permissões pelos botões da tabela.</p>',async f=>{await api('/tables',{method:'POST',body:{name:f.get('name'),columns:[],organization_isolated:f.has('organization_isolated')}});state.page='tables';state.table=f.get('name');});}
